function fixationSalience_and_significanceList(FIXATIONFILE,imageX,imageY,PLOTOPTIONS)
...
for s = 1:length(shuffunshuff)
    ...
    for cndlop=1:length(fixationstats)
        [saliencemap exe img] = getMaps(images(cndlop),RF);
        ...
        if ~isempty(fixationtimes)
            ...
            for i = 1:size(corrs,2)
                for ii = 1:size(corrs,3)
                    corrs{cndlop,i,ii} = NaN(1,numfixs);
                end
            end
            ...
            for i = 1:numfixs
                ...
                corrs{cndlop,1,1}(i) = saliencemap(spot(2),spot(1));
                corrs{cndlop,1,2}(i) = mean(saliencemap(spots));
                corrs{cndlop,2,1}(i) = exe(spot(2),spot(1));
                corrs{cndlop,2,2}(i) = mean(exe(spots));
                corrs{cndlop,3,1}(i) = img(spot(2),spot(1));
                corrs{cndlop,3,2}(i) = mean(img(spots));
            end
        end
    end
...
save([FIXATIONFILE(1:end-13) '-FixationStatistics'],...
    'shuffunshuffdata','statistics','statvariablenames','images')
    ...
    function [saliencemap exe img] = getMaps(salmapfile,RF)
        imgfile = [num2str(salmapfile) '.bmp'];
        salmapfile = [num2str(salmapfile) '-saliencemap.mat'];
        img = imread(imgfile);
        if size(img,3) == 1
            img = double(img);
        else
